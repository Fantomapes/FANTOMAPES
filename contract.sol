...
contract FantomApes is ERC721, Ownable {
    ...
    constructor(string memory name, string memory symbol, string memory baseURI,address payable _developer , address payable _marketer) ERC721(name, symbol) {
    ...
    function mintNFT(uint256 amount) public payable {
        ...
        uint developerAmount = (price * 40)/100;
        uint marketerAmount = (price * 60)/100;
        
        developer.transfer(developerAmount);
        marketer.transfer(marketerAmount);
    }
    ...
    function buyPunk(uint256 tokenId) payable public {
        ...
        pendingWithdrawals[seller] += msg.value * 94 / 100;
        // handle
        
        
        uint FeeAmount = msg.value * 6 / 100;
        
        developer.transfer(FeeAmount.div(2));
        marketer.transfer(FeeAmount.div(2));
        // handle
    ...
    function acceptBidForPunk(uint256 tokenId, uint256 minPrice) public onlyTradablePunk(_msgSender(), tokenId) {
        require(marketPaused == false, 'Market Paused');
        require(punkBids[tokenId].value > 0, 'there is not any bid');
        ...
        pendingWithdrawals[_msgSender()] += bid.value * 94 / 100;
        
        
        uint FeeAmount = bid.value * 6 / 100;
